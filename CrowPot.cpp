...
// 	return 0;
// }
// This code does not works for test cases like n = 3 k = 1 5 5 10 ans = 10 codeans = 15;
...
long long solve(int arr[], int n, int p)
{
    ...
    // we are checking it for every k size  window in the sorted array because by taking min pot only not give the right ans in many cases so we have to consider both cases
    // of taking min pot and max pot
    //input : {2,4,7,7,7,7,7,7,7} , K = 3, Actual Ans: 21 , if we going to remove 2,4,7 then ans is 42
    //input : {1,2,6,6,6,6,6,6,6,10} , K = 2, Actual Ans: 16 , if we going to remove 1,2,6 then ans is 19
   //input : {1,2,3,3,4,4,6,6,6,11} , K = 2, Actual Ans: 17 , if we going to remove 1,2,3 then ans is 19
   //input : {1,2,3,4,5,5,6} , K = 3, Actual Ans: 16 ,if we going to remove 1,2,3 then ans is 18
    for(int i=n;i>0;i--)
    {
        for(int z=1; z<=p;z++)
        {
